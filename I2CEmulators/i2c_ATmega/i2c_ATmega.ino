...
// Ivan: I2C changed some definitions
/*
 * Disabled Pull-Up (#define I2C_PULLUP 0)
 * Decreased timeout to 100 (instad of 1000) (#define 2C_TIMEOUT 100)
 * Enabled Fast Mode (#define I2C_FASTMODE 1) 
 */
...
byte *i2c_buffer1 = i2c_sensData;     // Entering buffer.
byte *i2c_buffer2 = i2c_sensData2;    // Out buffer.
...
// Request data from sensors by the master-mode i2c (SOFTWARE INTERFACE). NumBytes is the # of bytes to be readed from the line.
void i2c_Request(int NumBytes){
  ...
  for(int i=0; i<=(NumBytes)-2; i++){
      *(i2c_buffer1+i) = i2c_read(false);
      Serial.print(*(i2c_buffer1+i), HEX);
      Serial.print(" | ");
    }
...
void i2c_checkData() {
  ...
  if(check == *(i2c_buffer1+7)){      // check is turned into a consecutive XOR from 7 first bytes.
    ...
    Serial.println("RETR");
    i2c_Request(i2c_SENS_DATA_SIZE);}                  // if not, request for another transmission.
  } 
...
void i2c_empty() {
  ...
  //Serial.print("DATA A GUI: ");
//  Serial.print(i2c_sensData2[1] << 8 + i2c_sensData2[2]); Serial.print(" | ");
//  Serial.print(i2c_sensData2[3] << 8 + i2c_sensData2[4]); Serial.print(" | ");
//  Serial.println(i2c_sensData2[5] << 8 + i2c_sensData2[6]);
}
...
void i2c_SetParameters(int BytesNum){
  ...
  Serial.println((String)"GUI SP: " + i2c_receivedParam[0] + " | " + i2c_receivedParam[1] + " | " + i2c_receivedParam[2]);
}
